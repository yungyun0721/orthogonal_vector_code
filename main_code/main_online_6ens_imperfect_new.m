...
%     [test_u test_s test_v]=svd(metrix_ensemble(da2_run,1:7));
%     test_u6(times,1)=test_s(6,6);
%         da2_run = da2_run.add_record('analysis');
...
%    choose_i=setxor(1:7,romove_ens);
   ...
   ens7_aj_mean =mean(metrix_ensemble(da2_run,1:7),2);
   ens7_aj_pert =bsxfun(@minus,metrix_ensemble(da2_run,1:7),ens7_aj_mean);
