...
%      da2_run = da2_run.add_record('analysis');
    ...
    select_local=mod(k-select_localization:k+select_localization,40);
       ...
       k1=k1+5;
%        k1=6;
%% add new vector1 (vectical) (orth_IESV1)
%      cc=all_IESV(:,1);
     cc=mean(xb_final,2);
     cc=cc/sqrt(cc'*cc);
     vv=zeros(40,1);
    for j=1:5
        ens_proj_IESV1(:,j)=((u(:,j)'*cc)).*u(:,j); 
        vv=vv+ens_proj_IESV1(:,j);
    end
    vect=cc-vv;
    vect=vect/sqrt(vect'*vect);
    %% add new vector2 (vectical) (orth_ensmean)
     cc2=all_IESV(:,1);
%      cc2=mean(xb_final,2);
     cc2=cc2/sqrt(cc2'*cc2);
     vv2=zeros(40,1);
    for j=1:5
        ens_proj_IESV1(:,j)=((u(:,j)'*cc2)).*u(:,j); 
        vv2=vv2+ens_proj_IESV1(:,j);
    end
    vect2=cc2-vv2;
...
%         da2_run.ensmember{i}.vars{1}=da_run.ensmember{i}.vars{1}-spr_xb_initial.*u(:,k1)'/sqrt(42);
        da2_run.ensmember{i}.vars{1}=da_run.ensmember{i}.vars{1}-spr_xb_final.*plus_new_vector1'/sqrt(42)-spr_xb_final.*plus_new_vector2'/sqrt(56);
        end
%         da2_run.ensmember{7}.vars{1}=(mean(xb_initial(:,1:6),2)-spr_xb_initial.*u(:,k1))';
        da2_run.ensmember{7}.vars{1}=(mean(xb_final(:,1:6),2)+spr_xb_final.*plus_new_vector1*6./sqrt(42))'-spr_xb_final.*plus_new_vector2'/sqrt(56);
        da2_run.ensmember{8}.vars{1}=(mean(xb_final(:,1:6),2)+spr_xb_final.*plus_new_vector2*7./sqrt(56))';
...
%   da_run = select_refresh_ensmean(da_run,1:6);
  da2_run = select_refresh_ensmean(da2_run,1:8);
%% judgement/diagnose
